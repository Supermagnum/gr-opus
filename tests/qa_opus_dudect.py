...
class qa_opus_dudect(unittest.TestCase):
    ...
    def test_002_decoder_timing_independence(self):
        ...
        if welch_p is not None:
            self.assertGreater(welch_p, 0.01, f"Timing difference detected (p={welch_p:.6f}), potential side-channel")
    ...
    def test_003_bitrate_timing_independence(self):
        ...
        for bitrate in bitrates:
            encoder = opus_encoder(sample_rate=self.sample_rate, channels=self.channels, bitrate=bitrate)
        ...
        self.assertGreater(min_p, 0.01, f"Timing difference detected between bitrates (min p={min_p:.6f})")
