...
class qa_opus_performance(unittest.TestCase):
    ...
    def test_001_encoder_latency_10us(self):
        ...
        self.assertLess(mean_latency, 10.0, f"Mean latency {mean_latency:.3f}μs exceeds 10μs threshold")
    ...
    def test_003_realtime_voice_latency_0_02ms(self):
        ...
        encoder = opus_encoder(
            sample_rate=self.sample_rate, channels=self.channels, bitrate=64000, application="lowdelay"
        )
        ...
        self.assertLess(mean_latency_ms, 0.02, f"Mean latency {mean_latency_ms:.6f}ms exceeds 0.02ms threshold")

        # Verify 40ms budget is met (mean latency should be much less than 40ms)
        self.assertLess(mean_latency_ms, 40.0, f"Mean latency {mean_latency_ms:.6f}ms exceeds 40ms budget")
    ...
    def test_004_roundtrip_latency_40ms_budget(self):
        ...
        decoder = opus_decoder(sample_rate=self.sample_rate, channels=self.channels, packet_size=0)  # Auto-detect
        ...
        # Verify 40ms budget
        self.assertLess(mean_latency_ms, 40.0, f"Round-trip latency {mean_latency_ms:.6f}ms exceeds 40ms budget")
    ...
    def test_005_memory_stability_100_percent(self):
        ...
        self.assertLess(
            total_diff_mb,
            10.0,  # Allow up to 10MB for Python overhead
    ...
    def test_006_deterministic_behavior(self):
        ...
        print("  Iterations: 100 (fresh encoder each time)")
    ...
    def test_008_decoder_buffer_stability(self):
        ...
        self.assertLessEqual(
            max_buffer, decoder.max_buffer_size, f"Buffer size {max_buffer} exceeds limit {decoder.max_buffer_size}"
        )
